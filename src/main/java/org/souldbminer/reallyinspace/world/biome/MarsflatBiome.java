	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(MartianStoneBlock.block.getDefaultState(),
							MartianStoneBlock.block.getDefaultState(), MartianStoneBlock.block.getDefaultState())));
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(MartianLogBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(MartianLogBlock.block.getDefaultState()))).baseHeight(7)
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
											...
											setTreeBlockState(changedBlocks, world, blockpos, MartianLogBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												MartianLeavesBlock.block.getDefaultState(), bbox);
									}
								}
							}
						}
